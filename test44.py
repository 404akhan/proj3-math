...
total_comp = np.eye(drug_num, drug_num) * 55
...
### testing
for threshold in np.linspace(0.8, 1, 21):
	print ''
	correct_predicted = 0
	total_predicted = 0
	count_agree = 0
	...
	for sample_test in test:
		...
		if (y>0 and predict==1) or (y<=0 and predict==-1): count_agree += 1
		if predict != 0: total_agree += 1
	...
	print 'intersection', count_agree*1.0/total_agree
